...
def test_create_transaction_forbidden_account(test_users, logged_client, test_categories, test_accounts):
...
def test_update_transaction(logged_client, test_transactions, test_categories, test_accounts):
    updated_trans = {
        "title": "new_title",
        "amount": 20000,
...
def test_update_transaction_non_existent(logged_client, test_transactions, test_categories, test_accounts):
    updated_trans = {
        "title": "new_title",
        "amount": 20000,
        "category_id": test_categories[0].id,
...
def test_update_transaction_not_allowed(test_users, logged_client, test_transactions, test_categories, test_accounts):
    other_users_transactions = [
        trans for trans in test_transactions if trans.user_id != test_users[0]["id"]
    ]
    updated_trans = {
...
def test_update_transaction_category_only(logged_client, test_transactions, test_categories):
    updated_trans = {
        "category_id": test_categories[1].id
    }
    res = logged_client.put(f"/transactions/{test_transactions[0].id}", json=updated_trans)
